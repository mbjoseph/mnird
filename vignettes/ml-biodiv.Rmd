...
search_terms <- paste0("TS=(", 
                       ml_terms,
                       ") AND TS=(classif* OR identif* OR detect*) AND WC=(Ecology OR biodiversity conservation)")
...
method_tally <- abstracts %>%
  count(search_term) %>%
  separate(search_term, 
           into = c("method", "wos_topic", "wos_category"), 
           sep = " AND ", remove = FALSE) %>%
  mutate(method = gsub("TS\\=\\(", "", method), 
         method = gsub("\\)", "", method),
         method = gsub("-", " ", method),
         method = reorder(tools::toTitleCase(method), n)) %>%
  select(-wos_topic, -wos_category)
