...
search_terms <- paste(ml_terms, "AND (classif* OR identif* OR detect*)")

# Find the abstracts and clean them to prep for LDA
abstracts <- get_abstracts(search_terms)
write_csv(abstracts, "abstract-data.csv")
...
method_tally <- abstracts %>%
  ...
  separate(method, into = c("method", "blah"), sep = " and ")
p1 <- method_tally %>%
  ggplot(aes(n, fct_reorder(method, n))) +
  geom_point() +
...
p2 <- top_terms %>%
  ...
  xlab("") + ylab("")
p2
ggsave("fig2.png", p2, width = 6, height = 5)
...
p3
ggsave("fig3.png", p3, width = 8, height = 4)
```
